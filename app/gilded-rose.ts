...
export class GildedRose {
    ...
    updateQuality(): Item[] {
        for (let i: number = 0; i < this.items.length; i++) {
            ...
            if (this.items[i].name === itemNames.BackstagePasses && this.items[i].sellIn <= 0) {
                this.items[i].quality = 0;
                continue;
            }

            // Define sign to illustrate increasing or decreasing depending on product type
            const changeBySign: number = (this.items[i].name != itemNames.AgedBrie && this.items[i].name != itemNames.BackstagePasses) ? -1 : 1
            // Define weights to increase/decrease quality for items that are not a Backstage pass
            const weightNotBackstagePass: number = (this.items[i].name != itemNames.BackstagePasses && this.items[i].sellIn <= 0) ? 2 : 1
            // Define weights to increase/decrease quality for items that are a Backstage pass
            const weightBackstagePass: number = (this.items[i].name === itemNames.BackstagePasses && this.items[i].sellIn <= 5) ? 3 : (this.items[i].name === itemNames.BackstagePasses && this.items[i].sellIn <= 10) ? 2 : 1
            // Define the final weight
            const weight: number = (this.items[i].name === itemNames.BackstagePasses) ? weightBackstagePass : weightNotBackstagePass;
